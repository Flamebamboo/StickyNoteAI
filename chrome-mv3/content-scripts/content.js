var content=function(){"use strict";var be=Object.defineProperty;var he=(A,E,c)=>E in A?be(A,E,{enumerable:!0,configurable:!0,writable:!0,value:c}):A[E]=c;var I=(A,E,c)=>he(A,typeof E!="symbol"?E+"":E,c);var V,j;function A(e){return e}const c=(j=(V=globalThis.browser)==null?void 0:V.runtime)!=null&&j.id?globalThis.browser:globalThis.chrome,_={matches:["<all_urls>"],main(){console.log("🎯 StickyNoteAI v2.2 CSS FIXED + MENU POSITIONING - Loading..."),document.readyState==="loading"?document.addEventListener("DOMContentLoaded",()=>{Y()}):Y()}};let r=null,y=!1,W=!1,z={x:0,y:0},S={x:0,y:0},h=null;function Y(){console.log("StickyNoteAI: DOM ready, creating widget..."),K(),de(),ae(),le()}function K(){const e=document.getElementById("sticky-note-widget");e&&e.remove(),r=document.createElement("div"),r.id="sticky-note-widget";let t,n;try{t=c.runtime.getURL("smilyface.gif"),n=c.runtime.getURL("add2.png")}catch(u){console.warn("browser.runtime.getURL failed, using fallback approach:",u);const g=c.runtime.id||chrome.runtime.id;t=`chrome-extension://${g}/smilyface.gif`,n=`chrome-extension://${g}/add2.png`}console.log("StickyNoteAI: Image URLs:",{smilyFaceUrl:t,add2Url:n}),console.log("StickyNoteAI: Extension ID:",c.runtime.id),console.log("StickyNoteAI: Chrome runtime ID:",chrome.runtime.id),r.innerHTML=`
  ...
  `,document.head.appendChild(o),document.body.appendChild(r);const s=document.getElementById("smiley-image"),a=document.getElementById("add-image");s&&(s.addEventListener("load",()=>{console.log("✅ Smiley face image loaded successfully")}),s.addEventListener("error",()=>{console.error("❌ Failed to load smiley face image:",t),s.style.display="none"})),a&&(a.addEventListener("load",()=>{console.log("✅ Add2 image loaded successfully")}),a.addEventListener("error",()=>{console.error("❌ Failed to load add2 image:",n),a.style.display="none"})),J()}function J(){const e=document.getElementById("main-button"),t=document.getElementById("widget-menu");if(!e||!t)return;let n=0,o={x:0,y:0},s=!1;function a(i,l){if(!r)return{x:i,y:l};const d={width:50,height:50},b=window.innerWidth,x=window.innerHeight,p=10;let w=Math.max(p,i);w=Math.min(b-d.width-p,w);let k=Math.max(p,l);return k=Math.min(x-d.height-p,k),{x:w,y:k}}function u(i,l){if(!r)return{x:i,y:l};const d={width:50,height:50},b=window.innerWidth,x=window.innerHeight,p=20,w=i,k=b-(i+d.width),C=l,m=x-(l+d.height),f=Math.min(w,k,C,m);let L=i,N=l;return(i<0||i+d.width>b||l<0||l+d.height>x)&&(f===w?L=p:f===k?L=b-d.width-p:f===C?N=p:f===m&&(N=x-d.height-p)),{x:L,y:N}}e.addEventListener("mousedown",i=>{i.preventDefault(),n=Date.now(),o={x:i.clientX,y:i.clientY},s=!1;const l=r.getBoundingClientRect();z.x=i.clientX-l.left,z.y=i.clientY-l.top,e.classList.add("dragging"),document.addEventListener("mousemove",g),document.addEventListener("mouseup",v)}),e.addEventListener("mouseenter",()=>{y||(h&&(clearTimeout(h),h=null),H())}),t.addEventListener("mouseenter",()=>{h&&(clearTimeout(h),h=null)}),t.addEventListener("mouseleave",()=>{y||(h=setTimeout(()=>{T(),h=null},100))}),e.addEventListener("mouseleave",i=>{if(!y){const l=t.getBoundingClientRect(),d=i.clientX,b=i.clientY;d>=l.left-10&&d<=l.right+10&&b>=l.top-10&&b<=l.bottom+10||(h=setTimeout(()=>{T(),h=null},100))}});function g(i){const l=Date.now()-n,d=Math.sqrt(Math.pow(i.clientX-o.x,2)+Math.pow(i.clientY-o.y,2));if(!y&&(d>3||l>100)&&(y=!0,s=!0,T(),document.body.style.cursor="grabbing"),y){const b=i.clientX-z.x,x=i.clientY-z.y,p=a(b,x);r.style.transform=`translate(${p.x}px, ${p.y}px)`,r.style.left="0",r.style.top="0",S={x:p.x,y:p.y}}}function v(){if(document.removeEventListener("mousemove",g),document.removeEventListener("mouseup",v),e&&e.classList.remove("dragging"),document.body.style.cursor="",y){const i=u(S.x,S.y);i.x!==S.x||i.y!==S.y?(r.style.transition="all 0.3s cubic-bezier(0.4, 0, 0.2, 1)",r.style.left=i.x+"px",r.style.top=i.y+"px",r.style.transform="",setTimeout(()=>{r&&(r.style.transition="")},300),S=i):(r.style.left=S.x+"px",r.style.top=S.y+"px",r.style.transform=""),ce()}y=!1,setTimeout(()=>{s||H()},50)}t==null||t.addEventListener("click",i=>{const d=i.target.dataset.action;d&&(Q(d),T())})}function H(){if(y)return;const e=document.getElementById("widget-menu");e&&(e.classList.add("open"),W=!0)}function T(){const e=document.getElementById("widget-menu");e&&(e.classList.remove("open"),W=!1),h&&(clearTimeout(h),h=null)}function Q(e){switch(e){case"add":D();break;case"notes":te();break;case"settings":ne();break}}function D(e=""){Z(e)}function Z(e=""){const t=Date.now().toString(),n=document.createElement("div");n.className="sticky-note",n.id=`sticky-note-${t}`;const o=["rgba(255, 251, 147, 0.95)","rgba(255, 237, 213, 0.95)","rgba(237, 255, 235, 0.95)","rgba(235, 245, 255, 0.95)","rgba(255, 235, 255, 0.95)","rgba(255, 243, 205, 0.95)","rgba(243, 235, 255, 0.95)"],s=o[Math.floor(Math.random()*o.length)];n.innerHTML=`
  ...
  `,n.style.background=s;const a=(Math.random()-.5)*4;n.style.setProperty("--note-rotation",`${a}deg`),document.body.appendChild(n);const u=document.getElementById("sticky-note-widget");if(u){const g=u.getBoundingClientRect();n.style.left=Math.max(20,g.left-320)+"px",n.style.top=Math.max(20,g.top)+"px"}else n.style.left="100px",n.style.top="100px";return setTimeout(()=>n.classList.add("open"),10),ee(n),n}function ee(e,t){const n=e.querySelector(".sticky-note-header"),o=e.querySelector(".sticky-note-textarea"),s=e.querySelector(".close-btn"),a=e.querySelector(".minimize-btn"),u=e.querySelector(".pin-btn"),g=e.querySelector(".note-resize-handle");let v=!1,i=!1,l={x:0,y:0},d=!1,b=!1,x;o.addEventListener("input",()=>{clearTimeout(x),x=setTimeout(()=>{oe(o.value.trim())},1e3)}),n.addEventListener("mousedown",m=>{if(m.target.classList.contains("note-control-btn"))return;v=!0;const f=e.getBoundingClientRect();l.x=m.clientX-f.left,l.y=m.clientY-f.top,document.addEventListener("mousemove",p),document.addEventListener("mouseup",w),m.preventDefault()});function p(m){if(!v)return;const f=m.clientX-l.x,L=m.clientY-l.y,N=window.innerWidth-e.offsetWidth,me=window.innerHeight-e.offsetHeight;e.style.left=Math.max(0,Math.min(N,f))+"px",e.style.top=Math.max(0,Math.min(me,L))+"px"}function w(){v=!1,document.removeEventListener("mousemove",p),document.removeEventListener("mouseup",w)}g.addEventListener("mousedown",m=>{i=!0,document.addEventListener("mousemove",k),document.addEventListener("mouseup",C),m.preventDefault()});function k(m){if(!i)return;const f=e.getBoundingClientRect(),L=Math.max(200,m.clientX-f.left),N=Math.max(150,m.clientY-f.top);e.style.width=L+"px",e.style.height=N+"px"}function C(){i=!1,document.removeEventListener("mousemove",k),document.removeEventListener("mouseup",C)}s==null||s.addEventListener("click",()=>{e.classList.remove("open"),setTimeout(()=>e.remove(),300)}),a==null||a.addEventListener("click",()=>{b=!b,b?(e.classList.add("minimized"),a.textContent="+",a.title="Restore"):(e.classList.remove("minimized"),a.textContent="−",a.title="Minimize")}),u==null||u.addEventListener("click",()=>{d=!d,d?(e.classList.add("pinned"),u.style.background="#4CAF50"):(e.classList.remove("pinned"),u.style.background="")})}function te(){let e=document.querySelector(".notes-panel");e||(e=document.createElement("div"),e.className="notes-panel",e.innerHTML=`
  ...
  `,document.body.appendChild(e),setTimeout(()=>e.classList.add("open"),10);function t(){e.classList.remove("open"),setTimeout(()=>e.remove(),300)}(n=e.querySelector(".modal-close"))==null||n.addEventListener("click",t),(o=e.querySelector(".close-settings"))==null||o.addEventListener("click",t)}function X(){r&&(r.style.display="none"),document.querySelectorAll(".sticky-note").forEach(t=>{t.style.display="none"})}function O(){r&&(r.style.display="block"),document.querySelectorAll(".sticky-note").forEach(t=>{t.style.display="block"})}function G(){return!r||r.style.display==="none"?!1:window.getComputedStyle(r).display!=="none"}async function oe(e){try{const n=(await c.storage.local.get("stickyNotes")).stickyNotes||[],o={id:Date.now().toString(),content:e,timestamp:new Date().toISOString(),url:window.location.href};n.unshift(o),n.length>50&&n.splice(50),await c.storage.local.set({stickyNotes:n}),console.log("Note saved successfully")}catch(t){console.error("Error saving note:",t)}}async function F(){const e=document.getElementById("notes-list");if(e)try{const n=(await c.storage.local.get("stickyNotes")).stickyNotes||[];if(n.length===0){e.innerHTML='<div style="padding: 20px; text-align: center; color: #000000; font-weight: 500;">📝 No notes yet<br><small style="color: #9929EA;">Create your first note!</small></div>';return}e.innerHTML=n.slice(0,10).map(o=>`
    ...
    `).join(""),e.querySelectorAll(".note-item").forEach(o=>{o.addEventListener("click",()=>{const s=o.dataset.noteId,a=n.find(u=>u.id===s);a&&ie(a)})})}catch(t){console.error("Error loading notes:",t)}}function ie(e){var s,a,u,g;const t=document.createElement("div");t.className="sticky-modal",t.innerHTML=`
    <div class="modal-content">
      <div class="modal-header">
